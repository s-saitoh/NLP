...
def neco_lines():
    ...
    with open(fname_parsed) as file_parsed:
        ...
        chunks = dict() # idxをkeyにChunkを辞書に格納
        idx = -1
    ...
    for chunks in neco_lines():
        ...
        for chunk in chunks:
            ...
            for src in chunk.srcs:
                if len(chunks[src].get_case_prt()) > 0:
            ...
            chunks_include_prt.sort(
                key=lambda x: x.get_case_prt()
            )

            # 出力
            out_file.write('{}\t{}\t{}\n'.format(
                sahen_wo + verbs[0].base,    # サ変接続名詞＋を＋最左の動詞の基本形
                ' '.join([chunk.get_case_prt()\
