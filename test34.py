...
#1文ずつ辞書のリストを作成し抽出
list_a_no_b = [] #確認用の出現順リスト
lines = neko_lines()
for line in lines:
    if len(line) > 2: #形態素の数が2つより多くて（3以上）
        for i in range(1, len(line) - 1):
            if line[i]['surface'] == 'の'\
                    and line[i - 1]['pos'] == '名詞'\
                    and line[i + 1]['pos'] == '名詞': # 表層形がのでかつ前と後の品詞が「の」なら
                list_a_no_b.append(line[i - 1]['surface'] + 'の' + line[i + 1]['surface'])
                #出現順リストに追加
a_no_b = set(list_a_no_b) # 重複除去

#確認しやすいようにnouns_testを使って出現順にソートして表示
print(str(sorted(a_no_b, key=list_a_no_b.index)).decode("string-escape"))
